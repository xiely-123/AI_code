...
batch_size1 = 100
lr = 1e-3
os.environ['CUDA_VISIBLE_DEVICES'] = '5,6'   ####gpu选择
...
Train_list = np.loadtxt("./train.txt", dtype=str, delimiter=',')
devel_list = np.loadtxt("./test.txt", dtype=str, delimiter=',')
...
for epoch in range(start_epoch,num_epochs):
    ...
    for i in range(1):
        ...
        data,label = dataread.batch_data_read(data_path)
        ...
        #target = target.long()
        out = net(inputs1)
        loss = criterion(out,target) #####out*inputs1.double()
        ...
        print('Epoch[{}/{}],Process[{}/{}],loss:{:.6f},ACC:{:.6f},time:{:.3f},leanring_rate:{:.6f}'.format(epoch + 1, num_epochs, i + 1, int(len(Train_list)/batch_size)+1, loss.item(), running_corrects/len(label), train_time, param_group['lr']))
    running_corrects = torch.zeros(1).squeeze().cuda()
    ...
    for i in range(1):
        ...
        running_corrects += torch.sum(prediction == target)
        total += len(label)  
    print('Accuracy: %f'%((running_corrects/total).cpu().detach().data.numpy()))
